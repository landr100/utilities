...
class MyLogScraper(object):
	...
	def __init__(self, logfile_name, logfile_type=nil, start_line='', end_line=nil, warnings_list=[], errors_list=[], success_list=[],
		...
		self._logfile_basename = (basename(logfile_name) if logfile_name is not None and File('a').exists(logfile_name) else '')
	...
	def __repr__(self):
		...
		return "MyLogScraper(filename %r, filetype %r, debug %r, ignoring duplicates %r, pid %r)" % (self._logfile_basename, self._logfile_type, self._debug, self._ignore_dupes, self._pid)

	def __save_line(self, line):
		if self._retain_if_success:
			# some input might be url encoded, so decode it first
			from urllib import urldecode
			# self._saved_lines += [CGI::unescape(line), self._line_num]
			self._saved_lines += [unquote(line), self._line_num]
			return true
	...
	def __examine(self, line):
		...
		if self._examined_lines[line]:
			# return if self._ignore_dupes # skip this line
		...
		# self._errors_list.each do |errors|
		for errors in self._errors_list:
			# if line.match(errors):
			if re.search(errors, line) is not None:
				# log("error: line number #{self._line_num} : .#{line}." unless self._silent)
		...
		# return save_line(line) if found_something
		# TODO: make this one method
		if found_something:
			self.__save_line(line)
			return false

		# self._warnings_list.each do |warning|
		for warning in self._warnings_list:
			# if line.match(warning):
			if re.search(warning, line):
				# log "warning: line number #{self._line_num} : .#{line}." unless self._silent
				if not self._silent:
		...
		# self._success_list.each do |success|
	...
	def __line_is_start(self, line):
		# return (line.match(self._start_line))
	...
	def __scrape_line(self, line, found_start):
		...
		if found_start:
			...
			if self._debug: log("found first occurrence of start line at line number %s" % self._line_num) # if self._debug
			found_start = true
			self.__examine(line)
		return found_start
	...
	def build_cmd_line(self, silent=true):
		...
		cmd_line = "my-mail.py --subject='%s' " % self.__summarize()
		...
		if self._num_warnings + self._num_errors > 0:
			...
			for line in self._saved_lines:
				...
				dlog(line)
				fp.write("%s: %s" % (line[1], line[0]))
			fp.close()
		else:
			body = "examined "
			body += (str(self._num_examined) if self._num_ignored == 0 else "%d of %d" % (self._num_examined-self._num_ignored, self._num_examined)) + " lines "
			body += "in %s log file %s, found" % (self._logfile_type, self._logfile_name)
			if self._num_errors > 0:
				body += " %d errors (%s:%s) and %d warnings" % (self._num_errors, self._saved_lines.first[1], self._saved_lines.first[0], self._num_warnings)
			elif self._num_warnings > 0:
				body += " %d (%s:%s) warnings" % (self._num_warnings, self._saved_lines.first[1], self._saved_lines.first[0])
			else:
				body += " no problems"

			cmd_line += "--body='%s'" % body

			if not silent: log(body)
	...
	def __finish_scraping(self, send_mail, silent):
		...
		if send_mail: run_cmd(cmd_line) # if send_mail
		return cmd_line
	...
	def scrape(self, send_mail=false, silent=false):
		...
		# in_fp = (self._logfile_name[/\.gz/] ? Zlib::GzipReader.open(self._logfile_name) : File.open(self._logfile_name, 'r'))
